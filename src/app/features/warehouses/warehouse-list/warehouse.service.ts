...
export class WarehouseService {
  ...
  getAllWarehouses(status?: string): Observable<Warehouse[]> {
    ...
    return this.http.get<ApiEnvelope<Warehouse[]>>(`${this.warehouseApi}/all`, { params }).pipe(map(res => res.data ?? []));
  }
  ...
  getWarehouseById(id: string): Observable<Warehouse> {
    return this.http.get<ApiEnvelope<Warehouse>>(`${this.warehouseApi}/${id}`).pipe(map(res => res.data));
  }
  ...
  createWarehouse(payload: Partial<Warehouse>): Observable<Warehouse> {
    return this.http.post<ApiEnvelope<Warehouse>>(this.warehouseApi, payload).pipe(map(res => res.data));
  }
  ...
  updateWarehouse(id: string, payload: Partial<Warehouse>): Observable<Warehouse> {
    return this.http.put<ApiEnvelope<Warehouse>>(`${this.warehouseApi}/${id}`, payload).pipe(map(res => res.data));
  }
  ...
  updateStatus(id: string, status: string): Observable<Warehouse> {
    return this.http.patch<ApiEnvelope<Warehouse>>(`${this.warehouseApi}/${id}/status`, { status }).pipe(map(res => res.data));
  }
  ...
  deleteWarehouse(id: string): Observable<boolean> {
    return this.http.delete<ApiEnvelope<void>>(`${this.warehouseApi}/${id}`).pipe(map(() => true));
  }
