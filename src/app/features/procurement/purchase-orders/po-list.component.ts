...
export class PurchaseOrderListComponent extends BaseComponent implements OnInit {
  ...
  readonly statusFilterOptions = [
    ...
    ...this.statusOptions.filter(o => o.value !== null).map(o => ({ label: o.label, value: o.value })),
  ];
  readonly pageSizeOptions = DEFAULT_PAGE_SIZE_OPTIONS;

  // State signals
  readonly purchaseOrders = signal<PurchaseOrder[]>([]);
  readonly allLoadedPurchaseOrders = signal<PurchaseOrder[]>([]);
  ...
  readonly columnFilter = new ColumnTextFilter<PurchaseOrder>(() => this.allLoadedPurchaseOrders(), {
    status: 'equals',
    orderDate: 'contains',
  });
  ...
  selectedPoForDetail: PurchaseOrder | null = null;
  ...
  loadData(): void {
    ...
    this.purchaseOrderService
      ...
      .subscribe({
        next: res => {
          this.allLoadedPurchaseOrders.set(res.items);
  ...
  resetAllFieldFilter(): void {
    ...
    this.purchaseOrders.set(resetList);
    this.refreshCheckState();
  ...
  switchToEditFromDetail(): void {
    if (this.selectedPoForDetail) {
      const po = this.selectedPoForDetail;
      this.closeDetailModal();
      this.openEditModal(po);
    }
  }
  ...
  onDeleteSelected(): void {
    ...
    this.modalService.confirm({
      ...
      nzContent: `Bạn có chắc chắn muốn xóa <strong>${selectedIds.length}</strong> đơn đã chọn? Dữ liệu bị xóa sẽ không thể phục hồi.`,
      nzOkText: 'Xóa nhanh',
      nzOkDanger: true,
      nzCancelText: 'Hủy',
      nzOnOk: () => {
        this.purchaseOrderService
          ...
          .subscribe({
            next: () => {
              this.toastService.success('Thành công', `Đã xóa nhanh ${selectedIds.length} đơn đã chọn.`);
              this.clearSelection();
              this.loadData();
            },
            error: err => this.toastService.error('Lỗi', err.message || 'Không thể xóa các đơn đã chọn.'),
          });
      },
    });
  }
