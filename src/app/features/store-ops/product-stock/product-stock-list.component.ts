...
export class StoreProductStockListComponent extends BaseComponent implements OnInit {
  ...
  // Tham số kế hoạch ước lượng NVL (tab NVL & Cấp hàng)
  planDays = 7;
  safetyFactor = 1.5;
  ...
  /**
   * Badge chính: bán được hay không (quyết định bởi NVL, không phải kế hoạch).
   */
  stockMeta(s: PosDailyStock): { label: string; badgeClass: string; tooltip: string } {
    if (this.hasCapability(s) && (s.capabilityQuantity ?? 0) <= 0) {
      return {
        label: 'Tạm hết (NVL)',
        badgeClass: 'tbl-badge tbl-badge--danger',
        tooltip: 'NVL trong kho không còn đủ pha món này — cần xin cấp hàng',
      };
    }
    return {
      label: 'Đang mở bán',
  ...
  /** Hết bán được = NVL không còn đủ pha (quyết định bởi năng lực, không phải kế hoạch). */
  outCount(): number {
    return this.stocks().filter(s => this.hasCapability(s) && (s.capabilityQuantity ?? 0) <= 0).length;
  }
  ...
  // ── Tab NVL & Cấp hàng ─────────────────────────────────────────────
  // Đổi tab là tải lại số liệu tab đó (đơn bán xong quay lại là thấy số mới,
  // khỏi F5). Tab NVL tốn 1 API đối soát nên chỉ tải khi đang đứng tab 1.
  ...
  loadShortage(): void {
    if (!this.selectedBranchId) {
      this.toastService.warning('Chưa chọn chi nhánh', 'Vui lòng chọn chi nhánh trước khi xem ước lượng NVL.');
      return;
    }
    this.shortageLoading.set(true);
    this.shiftService
      .getMaterialShortage(this.selectedBranchId, {
      ...
      .subscribe({
        ...
        error: (err: Error) => {
          this.shortageLoading.set(false);
          this.toastService.error(err.message || 'Không tải được bảng ước lượng NVL.');
        },
      });
  }
  ...
  openRequestReplenishment(): void {
    ...
    this.modalService.confirm({
      ...
      nzContent: `Tạo yêu cầu xin cấp <strong>${count} NVL thiếu</strong> cho kế hoạch <strong>${this.planDays} ngày</strong> (an toàn ×${this.safetyFactor}) từ kho tổng (<strong>${s?.centralWarehouseCode || ''}</strong>) về kho quán (<strong>${s?.warehouseCode || ''}</strong>)? Kho tổng sẽ duyệt rồi mới xuất.`,
  ...
  private doRequestReplenishment(): void {
    ...
    this.shiftService
      .requestReplenishment(this.selectedBranchId, {
        planDays: this.planDays,
        safetyFactor: this.safetyFactor,
      })
      .subscribe({
