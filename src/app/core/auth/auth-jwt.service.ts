...
export class AuthServerProvider {
  ...
  private readonly API = SERVER_API_URL;
  ...
  login(credentials: LoginCredentials): Observable<ApiResponse<AuthResponse>> {
    ...
    return this.http.post<ApiResponse<AuthResponse>>(`${this.API}/api/v1/auth/login`, body);
  }

  refreshToken(refreshToken: string): Observable<ApiResponse<AuthResponse>> {
    return this.http.post<ApiResponse<AuthResponse>>(`${this.API}/api/v1/auth/refresh-token`, { refreshToken });
  }
